...
#include "hyp_debug.h"
//extern struct pkvm_module_ops *ops;
...
int read_buffer(void)
{
	...
	case HYP_DBG_CALL_PRINT_S2:
		ret = print_mappings(0, 0, 0, 1);
		break;
	case HYP_DBG_CALL_COUNT_SHARED:
		ret = count_shared(0, 0, 0, 1);
		break;
	default:
		ret = 0;
		break;
...
u64 hyp_dbg(u64 cmd, u64 param1, u64 param2, u64 param3, u64 param4)
{
	...
	case HYP_DBG_CALL_READ_BUFFER:
		...
		break;
	case HYP_DBG_CALL_PRINT_S2:
		ret = print_mappings(param1, param2, param3, param4);
		if (ret == 1)
			scmd = cmd;
		break;
	case HYP_DBG_CALL_COUNT_SHARED:
		hyp_print("call shared\n");
		ret = count_shared(param1, param2, param3, param4);
		hyp_print("call ret %x\n",ret);
		if (ret == 1)
			scmd = cmd;
		break;
//	case 4:
...
//	hyp_print("hyp_dbg ret %x %llx %lld\n",cmd,ret);
	return ret;
}
#if 0
