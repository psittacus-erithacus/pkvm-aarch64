...
//		hyp_print("hstpte %llx %llx\n", hwalker_data.pte, *ctx->ptep);
		...
		init_dbuf(&data->data, ctx->addr, ctx->level, ctx->ptep, hwalker_data.pte);
	} else {
		if (data->data.size) {
			if (print_dbuf(&data->data, data->hyp)) {
				data->fail_addr = data->data.start_ipa;
...
int count_shared(u32 id, u64 size, u64 lck, u64 cont)
{
	int ret;
	...
	if (ret == 1) {
		...
		//hyp_print("kvm_pgtable_walk ret 1 %llx\n",walker_data.fail_addr);
	...
	if (ret == 1) {
		//hyp_print("kvm_pgtable_walk ret2 1 %llx\n",walker_data.fail_addr);
		hyp_print("buff full in foot\n");
		sdata.addr = walker_data.fail_addr;
		sdata.share_count = walker_data.share_count;
		sdata.size = 0;
		return 1;
