...
//struct walk_data wdata;
u64 print_mappings(u32 id, u64 start, u64 size, u64 cont)
{
	int ret = 0;
	...
	//hyp_print("cmp %llx %llx\n", sdata.addr + sdata.size, (1UL << pgt->ia_bits) - 1);

	if (sdata.addr + sdata.size > (1UL << pgt->ia_bits) - 1) {
		sdata.size =  (1UL << pgt->ia_bits) - 1 - sdata.addr;
		//hyp_print("dec size by %llx to %llx\n",(1UL << pgt->ia_bits) - 1 - sdata.addr, sdata.size);
	}
	...
	if (ret == 1) {
		hyp_print("print mappings id %x addr %llx size %llx\n", sdata.id,
				sdata.wdata.start_ipa, sdata.wdata.start_ipa - sdata.addr);
		sdata.size -= sdata.wdata.start_ipa - sdata.addr -1;
		sdata.addr = sdata.wdata.start_ipa;
		//hyp_print("ret print_mappings %llx  id %x addr %llx, size %llx\n",&sdata, sdata.id, sdata.addr, sdata.size);

		return 1;//sdata.wdata.start_ipa >> 4;
	}//	else {
	//	memset(sdata ,0, sizeof(struct static_data));
	//	ret = 0; ///TODO study why
	//}
...
err:
	//hyp_dbg_print("%c%c",0x04,0x00);
	hyp_print("print_mappings end %lld\n",ret);
	return ret;
}
