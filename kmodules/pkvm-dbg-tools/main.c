...
static int device_open(struct inode *inode, struct file *filp)
{
	...
	buffer->ri = 0;
	buffer->wi = 0;
	buffer->size = DBG_BUFF_SIZE - sizeof(struct shared_buf);;

	if (ret)
		return ret;
	ret = pkvm_el2_mod_call(pkvm_mod_dbg_tools, HYP_DBG_CALL_INIT,
				virt_to_pfn(p), DBG_BUFF_SIZE, 0, 0);
	//ret = pkvm_el2_mod_call(128, HYP_DBG_CALL_INIT,
	//			virt_to_pfn(p), DBG_BUFF_SIZE, 0, 0);
	return ret;
}
...
/*
	case HYPDBG_PRINT_RAMLOG:
		   ret = pkvm_el2_mod_call(128, cmd & _IOC_NRMASK,
						p->dump, 0, 0, 0);
		break;
*/
	default:
